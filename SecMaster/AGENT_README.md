...
CROSS-SERVICE: collectors→register(f-a-f); ThresholdEngine→ResolveBatch(sync); Sentinel→resolve-entities(sync).
  OUT: collectors REST, OpenFIGI, Gemini, ollama-cpu-embed(embeddings), llama-cpu-rag(RAG generation). FEEDS: ThresholdEngine matrix via sector grounding.
  Generation runner = llama.cpp llama-server (llama-cpu-rag, qwen2.5:7b q4_K_M) since 2026-06-11; OllamaClient generation speaks /completion (raw prompt, no chat template; truncation=stop_type:limit ∨ legacy stopped_limit — `stop:true` even at cap, NOT a signal); Ollama__* names + IOllamaClient = config seam ¬engine. Swap rationale (rag-cpu-scaleout bench): ollama 0.24/0.30 bundled CPU runner decoded same GGUF at 0.2-1.3 tok/s vs llama.cpp ~24 tok/s (~30× ENGINE gap; HT/contention theory REFUTED, second-order ±30%). Embeddings UNCHANGED on ollama-cpu-embed /api/embeddings.
