...
## OPEN (current truth)
- 📝 **Service-docs refresh ✅ DONE (#653 merged `0196affe`, prompt-synced `ab183060`):** AGENT_README + README + ARCHITECTURE + EXEC-SUMMARY + deployment README updated to GPU-JSON-CoD/verifier-removed/continuous-streaming reality; live narrative PROMPT synced to host mount (`--tags sentinel-prompts`, News-momentum live, bias-vs-matrix gone). **Residual follow-up (low-pri, CODE comments not docs): 3 stale `SemanticVerifier` mentions (`IV2ExtractionPipeline.cs:56`, `CpuDslExtractionService.cs:245`, `SentinelMeter.cs:1004`) + `DependencyInjection.cs:53` "LlamaServerDsl=live prod" XML-doc → fold into the next SentinelCollector code change.**
