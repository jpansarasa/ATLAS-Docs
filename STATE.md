...
0. **FIRST TURN OF SESSION ONLY** — git audit BEFORE any dispatch: `git worktree list`, `git branch`, `git status` (in `/home/james/ATLAS`), `git -C /home/james/ATLAS fetch origin && git -C /home/james/ATLAS log --oneline HEAD..origin/main`. Flag stale `worktree-agent-*` branches/worktrees (from merged PRs), detached HEADs, dirty main, behind-main drift. Safe cleanup direct; destructive (worktree remove, branch delete, stash drop) needs user approval. Per `[[feedback_git_session_start_discipline]]` + `[[feedback_git_verify_each_step]]` + `[[feedback_main_always_current]]`.
...
- **Phase 5 matrix-integration arc MERGED (2026-05-26 → 2026-05-27)**: 8-sub-phase chain wiring `EnrichedDocument` through the Sentinel → ThresholdEngine → `matrix_cells` pipeline per `docs/plans/atlas-dsl-poc-phase5-matrix-integration.md`. **5.0 plan** (PR #477 `7dc9b72a`). **5.1 schema** — `matrix_cells` Sentinel-producer fields (PR #478 `5b120c66`; fixed by PR #480 composite-PK for hypertable). **5.2** `SignalMagnitudeCalculator` + `MatrixCellUpdate` — weighted-mean + daily UTC bucket (PR #482 `8db7efdb`). **5.3** `SignalFilter` — §8.2 safety-property emit gate (PR #483 `07c1661c`). **5.4** SentinelCollector matrix bridge — filter + calculator + emit channel (PR #484 `ec689f87`). **5.5** gRPC bridge `MatrixCellEnrichment` stream + `MatrixCellSentinelWorker` consumer in ThresholdEngine (PR #485 `a8d4da60`). **5.6** matrix-cell audit REST surface — by-doc + by-article projections (PR #486 `6f9ce9a0`; raw-SQL jsonb fix PR #487 `d430455b`). **5.7** `sentinel-matrix-pipeline` Grafana dashboard + alerts (PR #488 `bea3ce88`; silent-skip metric PR #490 `acb00128`). **5.8 acceptance demo** — 4 runs (#489 / #494 / #496 closed FAIL / #499 closed PROGRESS 3/10 / current PR #510 OPEN FAIL 0/10 on CPU CoD). Plan §5 step-2 EntityResolver SecMaster composer + macro signal-identity grounding (PR #491 `cf4ec94d`). Supporting fixes en-route: #492 secmaster exhaust-all-lookups, #493 EntityResolver per-stage OTEL metrics, #495 secmaster case-insensitive alias lookup, #497 prepass entity bag + MacroSignalIdentityCatalog expansion, #498 deterministic V2 extractor (T=0 + seed), #502 V2 prompt R6.1/R7/R7.1 hardening, #503 demo-harness sourceDocumentRef attribution fix, #504 drop `UseEntityResolutionPrepass` flag (PoC: no flags).
- **CPU CoD production cutover arc (2026-05-27)**: 6-PR chain implementing plan §5 — extraction moves CPU-side via llama-server + GBNF + DSL pipeline (per `[[atlas-inference-topology]]` rule). **PR 1/6** `LlamaServerClient` grammar field for GBNF-constrained decoding (PR #505 `3aeca9c1`). **PR 2/6** CPU DSL prompts (v8 baseline + v14 word-hints) + v2.3 GBNF deployed to `/prompts/cod` host mount (PR #506 `e0d7e7e1`). **PR 3/6** `dsl-parser-mcp` Python sidecar wrapping benchmark DSL parser + verifier (PR #507 `b552c104`). **PR 4/6** DSL AST → `MergedExtractionResult` adapter + `DslParserClient` (PR #508 `2868ba8e`). **PR 5/6** `CpuDslExtractionService` + `InferenceBackend.LlamaServerDsl` enum value (PR #509 `dc10a143`). **PR 6/6** compose flip `Extraction__Backend=LlamaServerDsl` + Phase 5.8 acceptance demo on CPU CoD (PR #510 OPEN, `dfdc69e1`, 14 polish commits pushed 2026-05-28: timeout fixes via #548, allow-list harness scaffolding, idempotency-prune in demo, EntityResolutionTimeoutSeconds 5→60, SemanticPipelineTimeoutSeconds 30→180; 1331 unit + 6 integration tests pass). Run9 verdict: **2 cells from 1/10 articles (FOMC 29807)** — §17.4 path A `cells>0` criterion MET; remaining 9/10 drop at safety filter (`entity_resolution_exhausted=61`, `gpu_verifier_none=28`) — same architectural class as §17.3, quality gap not architecture. CPU throughput unexpectedly lifted to 6-13 t/s (vs 3.67-4.25 baseline). Pipeline end-to-end functionally proven. **PR #510 squash-merged at `ff72eb8f` (2026-05-28T16:47:39Z)** — Phase 5 matrix-integration arc DONE. 18 commits squashed; 1333 unit + 6 integration tests pass; 0 critical / 4 important review findings closed in-PR. Tag `dsl-poc-phase5-done` + RELEASES.md + sub-plan prune dispatched per CLAUDE.md PHASE_TAGS. Phase 6 follow-ups (NOT blockers): remaining 9/10 filter drops (`entity_resolution_exhausted=61`, `gpu_verifier_none=28`), audit-provenance `rawContentId=null`, CPU throughput 4-13 t/s borderline for sync extraction.
- **README + system-docs audit campaign (2026-05-27)**: ~30 PRs reconciling docs against source-of-truth post-Phase-5 cutover. **28 service READMEs** corrected against source (PRs #512–#540: alphavantage/nasdaq/finnhub/fred/ofr/whisper/llm-benchmark/root/macro-substrate/reports/threshold-engine/finbert-sidecar/dsl-parser-mcp/calendar-service/edge/secmaster/sentinel-collector/alert-service/finnhub-collector-mcp/ntfy-mcp/ofr-collector-mcp/fred-collector-mcp/gemini-resolver-mcp/secmaster-mcp/threshold-engine-mcp/events/markitdown-mcp/whisper-service-mcp/deployment). **ARCHITECTURE.md** (PR #544 `9dc02c5e`) + **EXECUTIVE-SUMMARY.md** (PR #542 `f0d3061b`) reconciled against post-audit READMEs — converged on 79 patterns (not 62/68 stale claims), per-sector 5-band regime model on [-3,+3] (not single-score 4-state/6-state), Alertmanager flow (not direct HTTP), 4-service inference quartet. **README-TEMPLATE** restructured into 6 variants (SERVICE / MCP / SIDECAR / LIBRARY / INFRA / INDEX / EDGE) + router (PR #543 `625459db`). **3 defunct local-only directories** deleted (`WhisperServiceMcp/`, top-level `agent/`, `sentinel-mcp/`). **CLAUDE.md** SecMaster gRPC port corrected 8080 → 5001 in DATA_FLOW section (PR #541 `58abd237`).
- **Supervisor scaffolding meta-fix (2026-05-27, PR #511 `2c53bdc8`)**: supervisor-mode SKILL.md added `PLAN_GROUNDING` HARD_STOP + RED_FLAGS section; STATE.md gained "EACH TURN START HERE" anchor block referencing plan §5/§6/§7/§8; memory cleanup — `atlas-inference-topology` + `sentinel-llm-strength-layering` rewritten to match plan, new `feedback-benchmark-not-production` + `feedback-walk-pipeline-each-turn` memories added. Triggered after recurring "dispatch without re-grounding in plan" pattern + benchmark-score-as-production-capable misuse.
...
## OPEN ASKS (for user)
- **Phase 5.8 demo polish (PR #510 OPEN)** — fork decision needed: **(A)** ~3–4hr polish PRs to fix the 3 diagnosed gaps (DSL adapter `source_entity` emission, Gemini MCP fallback wiring into DSL adapter, CPU per-article timeout bump) and re-run demo; **(B)** rollback to GPU V2 via `git revert` + ansible; **(C)** accept architecture milestone + defer polish. Pending user direction per NTFY `z37K6aploLgM`.
...
- **README-TEMPLATE.md INFRA variant** — self-review caught + corrected invented ATLAS paths in PR #543; verify the 6 template variants (SERVICE / MCP / SIDECAR / LIBRARY / INFRA / INDEX / EDGE) are all routable from the README-TEMPLATE router. Low-priority maintenance.
