...
- **Phase 3 prompt iteration v3 → v4 → v5 → v6 + overnight diagnostics (2026-05-21→22)** — **v6 leaner PROGRESS; pivot DEFERRED.** Trajectory verifier (n=9): 0.790 (v3) → 0.808 (v4) → 0.782 (v5) → **0.895 (v6)**.
  ...
  - **Pivot to large-GPU DSL extraction track DEFERRED** per `[[sentinel-llm-strength-layering]]` memory updated 2026-05-22. v6 lifts the 4B-active MoE ceiling above pivot threshold (≥0.84). Next iteration options surfaced to user (NTFY `SpeBxHapl46Y`): v7 even leaner / v6 + context-aware classification / v6 + RAG few-shot / CoD layering.
- **Phase 4 GPU verifier deferred**: 3b-cheap (deterministic Python) remains the verifier path. Large-GPU emission track per layering memory deferred until v6+RAG also plateaus.
