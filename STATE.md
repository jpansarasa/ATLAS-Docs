...
- **A-track — data/backfill PRs MERGED + DEPLOYED; A3 cutover in review-fix.** #598 (sentinel level-as-rate guard), #599 (cpi/pce level→YoY), #600 (ALFRED vintages) merged; FredCollector + SentinelCollector redeployed. Verified 3/4 inflation families now write correct YoY% to `macro_observations` (cpi-core 2.5, pce-headline 2.9–3.8, pce-core 3.0–3.3). **cpi-headline-yoy still shows level (332.4)** — macro + cell writes are `ON CONFLICT DO NOTHING` and CPIAUCSL had pre-existing level rows → re-collection idempotent-skips. **HEAL-FIX (root cause, DML barred): `DO NOTHING`→`DO UPDATE` (last-write-wins) on macro_observations write + projector cell write** — also required so the authoritative matrix tracks FRED revisions; sequenced AFTER #601 (overlaps projector). **A3 cutover #601** (+600/−7354) reviewed APPROVE/PASS (deletion clean, flag-flip correct+loud, uses fixed `IsIdempotencyConflict`); fixing dead P1 alert + sentinel-matrix dashboard + dead `SentinelPolarityClassifier` + `Enum.IsDefined` → merge → **cutover DEPLOY** (projector→authoritative, drop legacy per-claim cells, user-accepted).
