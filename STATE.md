...
- **A-track (Option-A cutover) — ✅ COMPLETE + LIVE.** #598/#599/#600 + #601 all merged & deployed. **A3 cutover verified in prod**: `ObservationCellProjector` `Mode=Authoritative`, sole cell writer (55 signal-identity cells — cpi-headline-yoy / industrial-production / sahm-rule); **legacy Path-2 GONE** from running services; macro_observations (Path-1) intact; monitoring cleaned (dead P1 page removed). 3/4 inflation families write correct YoY% (cpi-core 2.5, pce-headline 2.9–3.8, pce-core 3.0–3.3). **Residue → heal-fix PR in flight** (`fix/ws3/heal-on-rewrite`): cpi-headline-yoy still clamped (stale 332) + projector logs ~220 `23505` Error/cycle — both fixed by macro+cell writes `DO NOTHING`→`DO UPDATE` (last-write-wins → clears the cell, tracks FRED revisions, kills the 23505 spam). 226 legacy NULL-keyed per-claim cells remain frozen (don't render in the signal-id matrix; DML barred).
- **B-track — ALFRED point-in-time vintages LANDED.** `POST /api/admin/backfill-vintages`: 470k+ true-vintage rows, 37+ series, AsOf **1927–2026**, multi-vintage confirmed. 6 daily market series (T10Y2Y/VIXCLS/DFII10/T5YIFR/RRPONTSYD/DCPN3M) hit FRED's 2000-vintage cap → no vintages, but market series aren't revised (≈nil impact; chunked requests = later #600 enhancement only if needed). **Backtest re-run DEFERRED** until matrix fully wired (post-cutover + heal-fix + pattern-blessing).
- **Follow-ons (post-cutover):** heal-fix PR (above); **pattern-blessing** — 3 corrected FRED YoY families project-skip "no pattern maps signal_identity_id", widen to unemployment-rate etc. (may need D1 weight sign-off); **C-track** Phase-4 projector-based sector×signal + disagreement dashboards (replaces deleted sentinel-matrix-pipeline). **NB self-push docs→main = SEPARATE commit + push bashes.** Orphan worktree `agent-ab81e556` needs user `sudo rm`.
...
- **Digest FULLY FIXED (#607/#608/#609, 2026-06-04, all deployed+verified):** the CoD cutover had silently zeroed `extracted_observations.Confidence` (`DslToMergedExtractionAdapter` NUM builder hardcoded `Confidence:0f`), so AutoApprove auto-rejected ~100% of news obs and the ntfy digest went empty since 05-29. Fixes: **#607** — adapter threads the per-block verifier verdict (IsPass→0.9, IsSoftFail/null→0.6) into both Confidence + ExtractionConfidence; digest orders CLAIM prose ahead of NUM fragments (verified live: post-deploy obs 0.9, Pending). **#608** — narrative payload capped at 80 obs (was assembling ~400 → HTTP 400 exceeds 32K ctx-size); + rejection-rate metric `sentinel_extraction_autoapprove_decisions_total` + alerts `SentinelExtractionAutoRejectHigh`/`SentinelDigestNarrativeMissing` (loaded in Prom). **#609** — narrative pinned to a dedicated GPU **VllmClient** (was on the ambient CPU `LlamaServerClient`, the `--parallel 1` slot extraction saturates → 180s timeout); now generates in ~7s, `llmStatus=Ok` (verified, digest 137). **Behaviour change to track: CoD news obs now persist `Pending` (was `Rejected`)** — Pending review queue grows with NUM fragments. Matrix path unaffected. See [[project_cod_cutover_zeroed_digest]].
  - **#610 — article-grounded narrative (deployed+verified 2026-06-05):** the narrative now summarizes the cleaned TEXT of the day's top ~25 articles (theme-stratified, signal-ranked; `DigestArticleSelector`) read from `raw_content.raw_file_path` → trafilatura → lede-heavy excerpt (`DigestArticleTextProvider`), anchored by the CoD values — instead of atomic ~10-char observation fragments. Client-side token cap (drops lowest-ranked articles < ~30K tokens; never relies on server reject). All-articles-unreadable → Degraded (alert fires); metric `sentinel_digest_narrative_articles_total{outcome=selected|read|read_failed|budget_dropped}`. Verified: trafilatura extracts rich prose from real rss articles; stale-window manual trigger correctly Degraded (TSA pages have no article body). Spec/plan in `docs/superpowers/`. Pattern: anything needing the LLM that isn't CPU-extraction uses a dedicated VllmClient.
  - **Historical backfill NOT done — mechanically ineffective (decided 2026-06-04):** re-extraction stamps `ExtractedAt=now` and the digest filters on `ExtractedAt`, so re-running old articles lands them in the CURRENT window (padding today's digest with stale news), NOT the historical [05-29→06-04] windows. The ~26k historical 0f/Rejected rows stay; only forward digests are healthy. Refilling history would need the digest to window on `collected_at` — not worth it for retrospective reports.
