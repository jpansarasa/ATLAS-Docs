...
- **CPU CoD production cutover arc (2026-05-27)**: 6-PR chain implementing plan §5 — extraction moves CPU-side via llama-server + GBNF + DSL pipeline (per `[[atlas-inference-topology]]` rule). **PR 1/6** `LlamaServerClient` grammar field for GBNF-constrained decoding (PR #505 `3aeca9c1`). **PR 2/6** CPU DSL prompts (v8 baseline + v14 word-hints) + v2.3 GBNF deployed to `/prompts/cod` host mount (PR #506 `e0d7e7e1`). **PR 3/6** `dsl-parser-mcp` Python sidecar wrapping benchmark DSL parser + verifier (PR #507 `b552c104`). **PR 4/6** DSL AST → `MergedExtractionResult` adapter + `DslParserClient` (PR #508 `2868ba8e`). **PR 5/6** `CpuDslExtractionService` + `InferenceBackend.LlamaServerDsl` enum value (PR #509 `dc10a143`). **PR 6/6** compose flip `Extraction__Backend=LlamaServerDsl` + Phase 5.8 acceptance demo on CPU CoD (PR #510 OPEN). Demo verdict: end-to-end pipeline functional but **0/10 cells emitted** — 3 diagnosed gaps remaining: (1) DSL adapter does not emit `source_entity`, (2) Gemini MCP fallback not wired into DSL adapter, (3) 3 articles TIMEOUT at current CPU latency budget. Decision fork pending (see OPEN ASKS).
