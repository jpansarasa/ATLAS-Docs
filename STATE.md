...
- **v15 compound regression (PR #429, `c2a292a8`, 2026-05-24)**: v14 word + chunked on n=72 REGRESSED to 0.7753 — RNG sampling variance dominates at n=72. Design goals MET (truncation 2/2, dup-ENT 2/4 recovered) but variance swamps signal. Chunked-merger ENT-dedup fix shipped regardless. **Two T=0 A/B sweeps IN FLIGHT** to isolate variance from design.
- **Production model**: v2.3 + v2.3.1 verifier is the leading production candidate (penalized 0.8733 beats β v12.1 penalized 0.8257). β v12.1 scored 0.887 still edges on gv-conditional. T=0 A/B will resolve whether v15 compound is RNG-noise-killed or design-killed. Hyp B LogitsProcessor remains the next architectural escalation when word-grounding plateaus (see `[[sentinel-llm-strength-layering]]`).
- **Phase 4 GPU verifier deferred**: 3b-cheap (deterministic Python) remains the verifier path.
