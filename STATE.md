...
## CURRENT WORK
*(idle)* — no active workstream. Get-to-clean-before-RAG bar is MET; collector/SecMaster restart baseline is clean (only intentional startup banners at Warning). **RAG experiment is the next queued item — unblocked.** For it: keep RAG OUT of any SecMaster readiness gate (degradable dep, not a readiness dep); the SecMaster RAG circuit-open WARN (llama-cpu-rag saturation) is the experiment's OWN target, not a system fault.
