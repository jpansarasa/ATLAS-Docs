...
### C. Open follow-ons
- ✅ **RAM EXPO → 5600 (DONE 2026-06-29).** Enabled via remote KVM. 6400 = manual OC (IMC/fabric tuning) with no remote CMOS-clear recovery → not worth the boot-risk from a 90-min-away box; 5600 = stable vendor EXPO profile, likely near the real 4×DDR5 ceiling anyway. Verified post-reboot: `dmidecode` = 5600, stack clean (`atlas.service` brought up 37 containers), `llama-cpu-rag` solo ≈ **29 tok/s** (was ~24, **+21%**) — lifts ALL CPU inference runners. Per-request still dips under concurrent IMC load → optional *safe* software lever `--parallel` 4→2 (no boot risk, fully remote-recoverable) if RAG-resolution latency ever matters.
