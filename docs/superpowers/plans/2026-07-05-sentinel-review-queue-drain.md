...
- [ ] **Step 5: Update `BuildObservationFromV2Result`** (`V2ExtractionPipeline.cs:265`): replace `var legacyConfidence = (float?)extraction.ResolutionConfidence;` with `var resConfidence = outcome.ResolutionConfidence;` and pass `resConfidence` to `UpdateResolution`. Update the nearby comment (WHY: res_conf must reflect the resolving leg, not the LLM self-report — the auto-approve gate and the `>= 0.8f` event-publish predicate at `ExtractionProcessor.cs:2120` both read it).
