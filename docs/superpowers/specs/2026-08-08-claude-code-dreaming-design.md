...
Every applied change appends to `dream/applied.md`: timestamp, run id, finding type, target, `eid=` (edit identity, the D-7 guard's key), `fp=` (claim identity, shared with `rejected.md`), and the claim. There is no diff, so the manual undo path is the SNAPSHOT; the ledger says what was applied, not how to reverse it.
...
When `DREAM_REPORT.md` holds items that are neither in the applied ledger nor already present in their target, SessionStart emits one line: the count and the first such item in REPORT order (not by severity -- the two coincide only because reports are written severity-ranked). It also announces a report the parser refuses, a missing report, and a report older than 36h, because each of those is otherwise indistinguishable from a clean night.
...
Plus apply-once (a second application of the same edit is REFUSED, by the ledger and independently by the target's own content -- not silently duplicated, and not a no-op; see D-7, which exists because the original "idempotence" claim here was false for every append-shaped finding) and a snapshot test (snapshot exists and matches pre-state).
...
D-2 snapshot-before-write: INTENT STATE.md has no git backup since #922, so the apply path is its only recovery mechanism / PRECOND a snapshot exists and is verified before the first write of any run, and its stamp is ISO 8601 basic UTC so pruning's sort-by-name is chronological; the stamp is ALSO retained as `_run_id` and is what D-7 stamps ledger entries with, so dropping that assignment silently unstamps the ledger -- do not simplify it away / GUARD `dream_apply.snapshot_or_refuse` plus the `_snapshot_taken` check at the head of `dream_apply.apply_finding` @ dream/dream_apply.py / TEST `dream/tests/test_apply_safety.py::test_apply_before_any_snapshot_refuses`, `::test_failed_snapshot_leaves_apply_closed`, `::test_snapshot_refuses_extended_format_stamp`
...
D-4 content-anchored-edits: INTENT the file moves between report time and review time, so position is not identity / PRECOND anchor text present verbatim exactly once, else refuse and re-queue / GUARD the occurrence check inside `dream_apply.content_refusal`, raised by `dream_apply.apply_finding` (citation follows the code; the check moved there under D-7, the precondition is unchanged). Its AMBIGUITY half runs BEFORE the content-evidence test and that ordering is load-bearing: behind it, an append-shaped edit -- whose replacement embeds its own anchor -- returned on content before the count ran at all, so a duplicated anchor was retired as "already applied" instead of re-queued to the only party who can disambiguate it. The ABSENCE half stays behind the content test, which is what keeps a destroy-shaped edit that already landed from being reported as drift @ dream/dream_apply.py / TEST `dream/tests/test_apply_safety.py::test_anchor_absent_refuses_and_leaves_file_identical`, `::test_ambiguous_anchor_refuses` (CORRECTION-shaped, so it reaches the count on either ordering and cannot see the subset above), `dream/tests/test_apply_semantics.py::test_an_ambiguous_anchor_is_re_queued_even_when_the_edit_shape_hides_it` (append-shaped, RED on the old ordering)
...
D-7 apply-once: INTENT anchor consumption was never an idempotency guarantee -- it holds only for replacements that DESTROY their anchor, and the NEW/INDEX shape embeds the anchor so the surrounding prose survives, which means re-applying appends the content a second time with no trace anywhere; measured on the 2026-08-09 report, 4 of 10 applied findings still matched their own anchor afterwards / PRECOND identity is the EDIT (`apply_identity` = type+target+anchor+replacement), NEVER the claim -- `fingerprint` strips digits, so two "STATE.md names a stale head" findings a week apart collide and claim-identity would refuse the second forever AND hide it from the notice; the edit must be absent from `applied.md` AND not already visible in the target, the second being what survives a lost ledger, a pre-D-7 entry and a crash before the record; the ledger entry is written by the apply path itself (never by a caller following prose) and only AFTER a write that landed, because ledger-first fabricated a successful apply out of an observed failure and `pending_findings` then hid that finding permanently; AND the apply path and the SessionStart notice consume ONE implementation of the content question -- `content_refusal` returns both the refusal to raise and whether the content PROVES the edit landed, and only the latter retires a finding from the notice, because "apply would refuse" and "there is nothing left to review" are different claims and re-deriving the first as the second from outside shipped a hiding regression in two consecutive rounds (round 2 hid every shortening edit, round 3 hid every drifted-and-never-applied finding, STATE.md being the highest-churn target); ONE implementation stops the two sides DISAGREEING and does nothing whatever for a verdict that is simply WRONG -- round 4's shared predicate read a drifted SHORTENING edit as applied, hid it from the notice and told the operator an untouched file was done, and the structural agreement test stayed green because both sides were wrong together. AND therefore the content half may retire a finding only when the replacement's presence could not PREDATE the edit: a shortening replacement is a substring of its own anchor, so it is in the body while the finding is pending and stays there after the anchor drifts, and applied-vs-drifted is not a question content can answer -- shortening edits are settled by the ledger alone. What remains admitted, and cannot be closed from the content side, is a non-shortening replacement that appears in the target by coincidence / GUARD `dream_apply.already_applied` plus `dream_apply.content_refusal`, whose only two call sites are `dream_apply.apply_finding` (raises `refusal.error`, after the D-2 snapshot, ledger and D-5 recheck gates) and `dream_apply.applied_on_content`, which `pending.pending_findings` consumes -- pending.py inspects no anchor, replacement or target content of its own @ dream/dream_apply.py, dream/pending.py / TEST `dream/tests/test_apply_semantics.py::test_append_style_finding_cannot_be_applied_twice`, `::test_a_recurring_claim_with_a_different_edit_still_applies`, `::test_an_edit_already_in_the_file_is_refused_with_no_ledger_at_all`, `::test_a_failed_content_write_records_nothing_and_stays_pending`, `::test_an_unreadable_ledger_entry_refuses_every_write`, `::test_the_notice_and_the_guard_consume_one_predicate` (structure only -- it cannot see a verdict both sides get wrong, which is why the two below pin verdicts), `::test_a_reconstructed_anchor_reads_the_same_to_both`, `::test_a_drifted_shortening_edit_is_refused_as_absent_not_as_applied`, `::test_a_shortening_edit_still_applies_once_and_is_then_ledger_refused`, `dream/tests/test_pending.py::test_a_drifted_anchor_that_never_applied_is_still_announced`, `::test_every_edit_shape_reads_the_same_to_the_guard_and_the_notice`
...
- never apply a finding the ledger already records; anchor consumption is not idempotency
- never decide "is this outstanding" from the report's `[applied ...]` mark; the ledger is truth
