# ATLAS `sectorWeights` Methodology (as-built)

*Status: **implemented**. The signed weight vectors specified here are deployed in the pattern JSONs under `ThresholdEngine/config/patterns/**` — as of 2026-07-18, **50 of 72 patterns carry non-zero vectors** (~22 remain all-zero: mostly `nbfi/`, `ofr/fsi-*`, and several `recession/*`). This is the as-built methodology reference; the live weights in the pattern files are authoritative (hot-reloadable, §E). The five review forks (sign convention + 7-level scale + no x3; cyclical-GDP composite; borderline CUTs kept; pure-macro reclassified) are settled — see the "Decisions" block and §F.*
...
- **`ThresholdEngine/src/Services/ObservationCellProjectorCore.cs`** is the **wired** matrix writer: `cell = magnitude x sourceTrust x freshness x temporal x confidence x sectorWeight`, clamped to `[-3, +3]` (`CellClampMin = -3m`, `CellClampMax = 3m`). `sectorWeight` is a plain `decimal` multiplier. **A negative weight flips the cell's sign relative to the signal** — exactly the "a signal is offensive for one sector, defensive for another" behaviour we want. There is **no `Math.Max(0, ...)` or `>= 0` guard anywhere on the path.** (The legacy `CellProjector.cs` / `patternWeight` formula is a **dead sibling** — driven only by the unwired `ObservationEventSubscriber` and the `/api/patterns/evaluate` endpoint; do not cite it for the matrix path. The signed-weight guarantee holds in both.)
...
- **Field shape** (confirmed in `recession/yield-curve-inversion.json`, `liquidity/credit-spread-widening.json`, and the `growth/housing-starts-to-*.json` split children): `sectorWeights` is a flat JSON object whose **keys are the 11 ALL-CAPS DB-form codes** and whose **values are JSON numbers** deserialised to `decimal`. As of 2026-07-18, 50 of 72 patterns carry non-zero vectors (the remaining ~22 are still all-zero).
...
- **Shape to write** (all 11 DB-form keys, signed decimals; example for `yield-curve-inversion` row C.1):
  ...
  ```
- **Hot-reload, no rebuild:** `ansible-playbook playbooks/deploy.yml --tags patterns`. The `PatternConfigurationWatcher` picks up the changed JSON; this is the fast tweak loop — edit a vector, deploy `--tags patterns`, watch the cell change in the sector x signal heatmap. No container rebuild needed.
...
2. **Scale granularity (A.3) — LOCKED: 7-level**, weights stay `[-1, +1]` (no x3 rescale). See §A.3.
