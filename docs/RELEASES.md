...
### `te-wm2ns-publication-cadence-done` (@ d912639a)
**WM2NS publication-cadence override — weekly-dated M2 published monthly** (2026-07-31). The new per-pattern daysOverdue alert (#897, `te-pattern-approaching-severe`) fired on deploy against `m2-growth-rate`/WM2NS. Digging in via FRED's API directly (series `observation_end`/`last_updated` + `fred/series/release` + `fred/release/dates`, curl'd from the host with the collector's key — the .NET container ships no curl) showed WM2NS is NOT stalled: it is weekly-DATED M2 published in the MONTHLY H.6 release (id 21, ~4th Monday) with a ~22-day lag. The system derives `PublicationFrequencyDays` from the series' DATA frequency (Weekly→7), giving a severe threshold `max(3×7,14)=21`, but WM2NS's healthy staleness oscillates ~15-42d every month → it would read severely-overdue ~3 weeks of every month (false Degraded + a monthly false P3). Verified WM2NS is the ONLY weekly FRED series with a monthly release (every other sits fresh at obs_end 07-24..07-30). A distinct cause class — publication-cadence ≠ data-frequency — NOT the #891 SecMaster drift (the data IS weekly), and relabelling the catalog Monthly would fight the #891 D-7 register-on-drift guard (self-reverting).
- **#898 per-pattern override** — optional `PublicationFrequencyDaysOverride` in the pattern JSON; the loader uses `Override ?? derived`. `m2-growth-rate` override=30 (monthly) → threshold `max(3×30,14)=90` > the ~50d max staleness → no false severe/alert. `GetYoY` is date-anchored (frequency-agnostic), unaffected. Guard tests: override wins (30) over derived (mock=1); a sibling proves the derive path still returns 1.
- **#899 alert-LHS coherence** — the deploy exposed that `data_overdue_days` (the alert's LHS gauge) re-resolved `maxFrequency` from SecMaster (=7) instead of `pattern.PublicationFrequencyDays` (=30 override), so LIVE it read 18 while the health check + threshold gauge honoured the override — the alert's LHS and RHS on different frequency bases (not-firing for m2 only by luck of the numbers). Fixed to use `PublicationFrequencyDays` (removed the now-dead `ISeriesFrequencyResolver` dependency + its redundant per-eval SecMaster round-trip).
- **#900 stale integration tests** — 4 `--integration` tests silently red since #231 (the default gate skips `--integration`, which needs a DB): 3 asserted the retired `sahm-rule` pattern (renamed `sahm-rule-official`, now FRED's official `SAHMCURRENT` series), 1 fixture omitted the now-mandatory 11 `sectorWeights` (D-4). Repaired → 61/61 green.
- **#901 comment-rot sweep** — `RoslynExpressionCompilerTests` comments named retired patterns (`sahm-rule.json` ×2, `vix-deployment-l1.json` — the last also falsely "the most complex surviving expression in our configs"); relabelled by the C# construct each exercises (expressions unchanged — valid compiler test vehicles regardless of provenance).

Deploy/verify (2026-07-31): TE built `--no-cache` + scoped ansible (`--tags threshold-engine`, which also mirrors the host-mounted patterns) for #898 then #899; both container-recreated. Verified LIVE through Prometheus: after #898, `severe_overdue_threshold_days{m2}` **21→90** and the alert query stopped matching m2; after #899, `data_overdue_days{m2}` **18→-5** (fresh vs its 30d cadence) — all three surfaces now agree (overdue=-5, threshold=90, health Healthy). `get_pattern` confirmed `PublicationFrequencyDays=30, SignalDecayDays=180`. 0 Loki Error/Fatal post-deploy each time. #900/#901 test-only (no deploy).

Lessons: the alert earned its keep on day one by surfacing a real mis-model, but the fix was incomplete until every surface shared ONE frequency basis — the health check honoured the override while the alert's own LHS gauge silently re-derived from SecMaster, an inconsistency invisible until the DEPLOYED gauge was read (verify the metric live, not just the code). Durable rule: for any series whose RELEASE cadence is coarser than its DATA frequency, set `publicationFrequencyDaysOverride` to the true release cadence — never relabel the catalog frequency (the data frequency is correct, and #891 D-7 would revert it).

### `te-pattern-data-health-done` (@ 0d5198fa)
**ThresholdEngine pattern-data health — false-positive triage, surface unification, stall alert** (2026-07-24 → 07-31). The post-power-cycle recovery check found TE pinned at Degraded/Unhealthy on the `pattern-data` health check. The through-line across the epic: **a "severely overdue" pattern is not always real staleness** — four distinct non-stall causes were each false-paging, the two health surfaces had silently drifted, and nothing actually alerted on a genuine stall. Root-caused one cause at a time, evidence-first (every "overdue" claim checked against the collector's real observation dates + FRED cadence before touching code — twice the first-suspected culprit was wrong).
- **#889 seriesless false-positive** — `CalculateFreshnessAsync` iterated `RequiredSeries` for the oldest observation; a pattern with `RequiredSeries: []` never entered the loop and fell through to "missing all data", so the 2 Sentinel news-breadth patterns (fed by the news pipeline, not a macro series) pinned the whole service Degraded every run. Guard the empty-series case → report current (no macro freshness to assess).
- **#890 FRED daily-on-monthly cron** — 3 daily series (ECBDFR, BAMLH0A1HYBB, BAMLH0A3HYC) fired only on the 1st/10th/20th (frozen up to 11d), surfacing as severely-overdue patterns. Onboarded before the D-1 frequency-normalize fix, so a "Daily, Close" string mis-derived Monthly → cron `0 0 10 1,10,20 * ?`; the prior `FixSeriesConfigFrequency` migration fixed the Frequency column but not the persisted CronExpression. Data-only EF migration → `0 0 18 * * ?` for the 4 rows (IGREA folded in). (The FX series first suspected were never stalled — FRED-side data lag.)
- **#891 SecMaster mapping drift (D-7)** — `fed-funds-rate` read pubFreq=1 for MONTHLY FEDFUNDS → a normal ~2-month value flagged 59d severely-overdue. The 07-18 collector frequency correction reached the collector DB + catalog but NOT the persisted `source_mapping`, because register was INSERT-ONLY for an existing mapping. D-7: register now UPDATES a mapping's frequency on real drift (case-insensitive; blank never clobbers; unchanged = no write) + a one-time `HealFredSourceMappingFrequencyDrift` migration corrected all 10 drifted rows. (Follow-up **#892** open: blank-frequency still persists on register INSERT paths — source-side clean.)
- **#893 grace floor for batch-published FX** — 6 FX/dollar patterns flagged severe. DTWEXBGS/DEXUSEU/DEXJPUS/DEXCHUS are daily observations published in lagged weekly batches (Fed H.10), routinely ~7d stale; with pubFreq=1 the bare `daysOverdue > pubFreq*3` false-paged at 3d. Severe now `daysOverdue > max(pubFreq*3, 14)` — daily series get a 2-week floor (tolerates batch/holiday lag, still catches a >14d stall); longer cadences unaffected (the floor is a MAX, not a MIN).
- **#894 slightly-overdue is informational** — TE read Degraded from `SlightlyOverdue > count*0.2` even with 0 severe / 0 missing. That trigger is un-tunable: daysOverdue is measured from the period-start-dated observation, so monthly/quarterly series read ~1 cycle overdue when perfectly healthy (~90% steady-state baseline; measured FEDFUNDS/CPIAUCSL/UNRATE/PAYEMS=30d, GDP=31d). Dropped slightly-overdue from the status determination; Degraded/Unhealthy now driven ONLY by NoData (missing) + SeverelyOverdue (beyond the grace floor). Slightly-overdue stays visible (data/issues + per-pattern Warning + the Healthy message). TE reads Healthy; a genuine stall still escalates slight → severe.
- **#896 unify the two surfaces** — `GET /api/patterns/health` was a SECOND copy of the classify+status logic that had drifted from the `IHealthCheck` probe on all three fixes above (missing the #889 skip → Unhealthy; bare `pubFreq*3` with no #893 floor; still gated on #894 slightly-overdue) — latent because it's wired to nothing. Extracted `PatternDataHealthEvaluator.EvaluateAsync` as the single source of truth; both surfaces map its `PatternDataHealthReport`. Metrics + severe-overdue Error logs emit ONLY from the probe (never the evaluator or the endpoint → not request-driven or double-counted). Both verified agreeing.
- **#897 per-pattern stall alert (closes #895)** — after #894 a genuine stall stays Healthy until it crosses its per-pattern severe threshold (14d daily / 90d monthly / 270d quarterly) and nothing watched the metric. Emit `severe_overdue_threshold_days{pattern_id}` from the SAME code that flags severe (`SevereOverdueThresholdDays` private→internal, no PromQL formula duplicated); Grafana RULE-ONLY alert `te-pattern-approaching-severe` fires P3/warning per-pattern at `data_overdue_days >= 0.8× threshold` (0.8× sits above every healthy baseline — monthly is ~30d overdue when healthy, 0.8·90=72), catching slight→severe BEFORE the status flips; `noDataState=OK`.

Deploy/verify (2026-07-24 → 07-31): each fix built `--no-cache` + scoped ansible (`--tags threshold-engine` / `fred-collector` / SecMaster; #890/#891 EF migrations applied on deploy; the #897 alert via `--tags alerting` — which regenerates compose and cascades a full-stack restart — plus a manual `grafana` restart, since alerting provisioning loads at startup only and grafana lives in the separate OTEL stack). Verified LIVE through Prometheus/`/health`: TE reads Healthy end-to-end (both surfaces agreeing), the threshold gauge (71 series, values 14/21/90/270) and the alert rule present and evaluating. **On #897's deploy the new alert immediately (correctly) surfaced `m2-growth-rate`/WM2NS** — a real mis-model that became its own chain (`te-wm2ns-publication-cadence-done`).

Lessons: the recurring failure mode was reading "severely overdue" as "collection broken" — four times it was something else (seriesless-by-design, batch publication, structural period-start dating, stale pubFreq mapping), so the EVIDENCE_GATE (check the collector's real dates + FRED cadence BEFORE theorising) is what discriminated each. And two consumers of one classification WILL drift into a half-fix unless unified at the source (#896) — the same disease as the correction-not-propagated-to-the-other-column pattern in #890/#891.

### `runtime-loglevel-rollout-done` (@ 7b56f5fa)
**Runtime-adjustable log level across the .NET fleet** (2026-07-22). The fleet is WARN-quiet by design -- liveness is proven by telemetry/metrics, not log lines -- so there was no on-demand way to raise a service to Info/Debug for a live debug session without a redeploy. This adds that: a Serilog `LoggingLevelSwitch` becomes the sole level authority, seeded from `Serilog:MinimumLevel:Default` (steady state Warning) and re-applied on config reload via an explicit `ChangeToken.OnChange` (Serilog.Settings.Configuration does NOT auto-reapply the switch), with the MEL `SetMinimumLevel` floor lowered to Debug so the logging bridge does not pre-filter below the switch. Applied inline per-service (no shared logging lib), each with a business-outcome guard test that goes RED if `ControlledBy`, the ChangeToken re-parse, or the MEL floor is removed. Mechanism: an operator raises the level by editing the level key IN the running container (`nerdctl exec`); `reloadOnChange` picks it up live with NO restart; the edit lives only in the container writable layer, so a RECREATE (not a restart -- a restart preserves the layer) reverts to the baked-in Warning.
- **#883 FredCollector (pilot)** -- established the pattern; mechanism PROVEN end-to-end on the real deployed container (exec-edit -> Debug in Loki, no restart -> recreate reverts). An in-container overlay edit fires .NET inotify on containerd/overlayfs.
- **#884 Finnhub / AlphaVantage / Ofr** -- collectors sharing the MEL-floor shape; verbatim copy of the pilot helper.
- **#885 Nasdaq / Sentinel / ThresholdEngine** -- Nasdaq code-only (disabled in prod). ThresholdEngine needed a Serilog.Settings.Configuration + Extensions.Hosting 8->9 bump so the version citation is truthful; review verified SAFE (TE already on Serilog core 4.2.0, so no core-major jump, no downgrade).
- **#886 AlertService / SecMaster / CalendarService (HTTP)** -- these use `UseSerilogRequestLogging`, so the pilot's removal of `Host.UseSerilog()` would have silently broken request logging (de-registers the concrete `DiagnosticContext` the middleware resolves). The pattern was ADAPTED to re-register `DiagnosticContext` + `IDiagnosticContext` wired to the switch-controlled logger (faithful to Serilog's `ConfigureDiagnosticContext`); verified by reflection in review and live at deploy (request-completion log still emits with enrichment). Serilog.AspNetCore 8.0.3->9.0.0 on Alert/Calendar, safe (core already v4).
- **#887 Reports Daily/Weekly/Monthly** -- scheduled-job hosts. Blocked by a PRE-EXISTING NU1903 advisory on System.Security.Cryptography.Xml 10.0.8 (transitive via MacroSubstrate) that Reports' `TreatWarningsAsErrors` promotes to a hard compile error; owner chose Reports-only remediation, so the pin was bumped 10.0.8->10.0.10 (clears the July 2026 EncryptedXml advisories; MacroSubstrate + fleet pins left for a later fleet-wide standardize). Review also caught a false "9.0.0" version citation (Reports actually resolve Settings.Configuration 8.0.4) -> corrected.

Scope: 13 .NET services; MCP sidecar hosts deliberately EXCLUDED (they hard-code the level with no appsettings/`ReadFrom.Configuration`, and are thin proxies with low debug value). Deploy/verify: every batch reviewed 0-critical / 0-important across code + intent + observability lenses, and each batch's review caught a real issue (the TE bump safety, the HTTP request-logging DI, the Reports version citation) rather than rubber-stamping. Deployed via scoped ansible (`--tags <svc> --skip-tags build`); the ATLAS stack was never restarted wholesale.

Lessons: the recurring failure mode was a green check that tested something ADJACENT to the property depended on -- a copied comment citing the wrong package version, a guard test that proves the switch but not the request-logging it preserves -- so per-service prechecks (does this service actually fit the pattern?) and cross-lens review (the intent reviewer cross-checking `project.assets.json`) caught what compile-plus-tests could not.
