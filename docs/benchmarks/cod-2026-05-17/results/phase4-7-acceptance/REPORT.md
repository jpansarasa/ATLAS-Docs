...
_Generated: 2026-05-25T12:30:35Z_
...
- Per-CLAIM verdicts compared: **137** (agree=48, disagree=89, foundry-missing=0)

## 2. Acceptance gate (§7.3, ≥85% agreement)

**Result:** **FAIL** — agreement = 0.3504 (35.04%); gate = ≥85%
...
- **production=none / foundry=partial:** 61
- **production=full / foundry=partial:** 16
- **production=none / foundry=full:** 6
- **production=partial / foundry=none:** 2
- **production=full / foundry=none:** 2
- **production=partial / foundry=full:** 2
...
- Production arm (vllm-server, `sentinel-cove-v6.2`): **$0.0000** (production runtime = $0 per plan §6).
- Production-arm total wall-time across n=50: **129515 ms** (129.5 s for 137 CLAIM calls).
...
- vllm-server today serves model id `sentinel-cove-v6.2` (Qwen2.5-32B-AWQ); the C# `ClaimVerifier.ProductionModel = "qwen2.5:32b"` literal 404s on this box. This run pins `sentinel-cove-v6.2` so the acceptance measurement reflects what production WILL do once the production model literal is reconciled to the served name — same weights, same prompt, same parser.
