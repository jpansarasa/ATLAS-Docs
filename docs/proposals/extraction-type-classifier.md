...
`EntityResolutionService.cs:784-794` — the `SignalIdentityDirect` return:
...
| economic-series → FRED search | `CatalogService.cs:148` | **NO — deliberately off.** `EntityResolutionService.cs:851` passes `allowEconomicDiscovery: false` ("a news surface is never a FRED economic series"). This is #818's fix; re-opening it per-type would need a named supersession |
...
(`SecMaster/AGENT_README.md:71`: "a call that never lands leaves the resolver ledger idle, not
