...
documented as such (`SentinelMeter.cs:750-754`: *"Bounded — no tags, scalar counters only"*). Both
...
`exact_rejected_name` (`DeterministicResolver.cs:416`) is a *status tag* on a resolver counter, not a
...
5. **`exact_rejected_name` as a criterion.** It is a status tag at `DeterministicResolver.cs:416` `[M]`,
