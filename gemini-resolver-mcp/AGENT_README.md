...
`--tags alerting --skip-tags always`, which copies and HUPs Prometheus), so "not ansible-deployed"
...
GOTCHAS:
  ...
  ✗ hand the SDK a stock httpx transport or client # connect.py is what makes the 25s connect
    timeout ONE deadline across DNS's 8 AAAA + 8 A; the stock backend gives EACH address 25s, so
    an unanswering AAAA path cost 200s per new connection on 2026-09-22. A dead family now costs
    a 250ms stagger, visible only in gemini_resolver_connect_fallbacks_total + a throttled WARNING
