...
**A service-scoped deploy ships NO dashboards, so a PR touching CODE AND a dashboard needs TWO invocations.** The six tasks that ship the monitoring tree — the `Deploy monitoring directory` copy plus its legacy-dashboard and orphaned-alert prune chain — carry the tags `monitoring`, `dashboards`, `alerting`, `otel`, not one service tag among them, so `--tags {service} --skip-tags build -e "scoped_restart=true scoped_services={service}"` cannot ship a dashboard edit, and nothing fails: the service deploys, the panel keeps querying the metric the PR retired. Measured 2026-09-20 after #1073 deployed: both dashboards it repointed (`Collectors & Services/fredcollector.json` and `Overview/atlas-home.json`) still queried the retired `fredcollector_events_published_total` on the host until `--tags dashboards --skip-tags always` ran. `{Project}/.devcontainer/build.sh` prints only the service-tag form, so the deploy hint does not carry this either. Derive the tag set rather than recalling it, from `deployment/ansible/`: `python3 -c "import yaml; ts=yaml.safe_load(open('playbooks/deploy.yml'))[0]['tasks']; d=[t for t in ts if 'dashboards' in (t.get('tags') or [])]; print(len(d), sorted({g for t in d for g in t['tags']}))"` -> `6 ['alerting', 'dashboards', 'monitoring', 'otel']`. Dashboards then auto-reload (`updateIntervalSeconds: 30`); alert rules need the Prometheus HUP the `alerting` tag already does, and Grafana alerting needs the restart below.
...
│   │   ├── ups-exporter/            # UPS exporter, apcupsd NIS -> :9144 (built by compose.otel.yaml.j2)
...
Prometheus, Alertmanager, Loki, Tempo, and the OTEL collector are intentionally NOT host-mapped — Grafana proxies queries to them across the `ai-inference` container network. The node and gpu exporters are scraped over that same network. `ups-exporter` is the exception: it runs `network_mode: host` to reach apcupsd's NIS socket on `127.0.0.1:3551`, so Prometheus scrapes it at `mercury:9144`.
