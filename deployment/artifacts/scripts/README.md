...
| `registry-metrics.sh` | `atlas-registry-metrics.timer`, every 5 min | Writes `atlas_registry.prom` for `node_exporter`: registry presence of each locally-held `127.0.0.1:5000/<image>:latest`, registry reachability, and a write timestamp. The header says what each series cannot see; the alerts are `monitoring/alerts/registry.yml`. |
