...
### Dual-write gate: the per-signal `matrix_benchmark` flag

Whether a series dual-writes observations into the shared `macro_observations` substrate (owned by `MacroSubstrate/`) is decided at collection time by the per-signal `matrix_benchmark` flag SecMaster carries on the series' resolved `SignalIdentityId` — there is no per-series opt-in flag on this collector. A series dual-writes when its `SignalIdentityId` resolves to a signal whose `matrix_benchmark` flag is `true`. The flag is **per-signal and orthogonal to `category`** — a benchmark signal can be any category (macro, rate, commodity, fx, credit, …), so non-macro daily series such as `oil-price`, `ust-10y-yield`, and `dxy-dollar-index` reach the matrix too. A series with a null or unresolved `SignalIdentityId` is never a matrix benchmark and never dual-writes. When a series is a matrix benchmark, every collected observation is also written to `macro_observations` with provenance `(source_collector="fred", source_id=<series mnemonic>, observation_time)`; the write is idempotent on that triple, so re-running a collection cycle does not double-write. The legacy `fred_observations` write path is unchanged — the substrate write is additive.

The matrix-benchmark id-set is owned by SecMaster, not configured here. `SecMasterMatrixBenchmarkClassifier` fetches it from SecMaster's `/api/signal-identities/matrix-benchmarks` endpoint and caches it (15-minute TTL) so the per-series collection path makes at most one round-trip per TTL window. The cache holds **last-known-good**: a transient SecMaster failure with a populated cache retains the previously-loaded set rather than flapping the gate. A **cold-start** fetch failure (no cache yet) yields an empty set, so the gate **fails closed** — no series dual-writes until SecMaster recovers — and the failed fetch is not cached, so the next collection retries. A successful fetch that returns **zero** ids is also served (the gate is closed for all series) but logs a warning and is tagged distinctly in telemetry, so benchmark-absence is observable rather than silent.

Series that are not matrix benchmarks (their resolved signal-identity has the flag `false` or unset) never dual-write; the per-series sector tag and signal-identity remain authoritative for that path.
...
- **MacroSubstrate dual-write** — When the series' resolved `SignalIdentityId` carries SecMaster's per-signal `matrix_benchmark` flag (fetched and cached by `SecMasterMatrixBenchmarkClassifier`), observations are additionally written to `macro_observations` for cross-collector substrate queries.
...
| `SECMASTER_REST_ENDPOINT` | SecMaster REST endpoint (used by both sector and signal-identity taggers **and** the `matrix_benchmark` dual-write gate). When unset, both tagger workers no-op **and** the gate is wired to `NullMatrixBenchmarkClassifier`, which classifies nothing as a matrix benchmark — so no series dual-writes to `macro_observations`. | unset → no-op |
...
- [MacroSubstrate](../MacroSubstrate/src/MacroSubstrate/README.md) — Shared substrate consumed by the `matrix_benchmark` dual-write path
