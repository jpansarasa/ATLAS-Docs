...
PURPOSE: market-data collector (commodity/economic/equity-OHLCV/forex/crypto) → AlphaVantage HTTP → TimescaleDB; emits ObservationEventStream gRPC. ¬identity/classification (that's SecMaster) ¬price-API for callers (poll-pull via REST/repo, ¬push values).
...
DATA MODEL + INVARIANTS:
  ...
  INV quota in-process⊥persisted: dailyCount + token-bucket = in-memory; resets at UTC-midnight roll OR on restart. ¬DB-backed → restart = quota wiped.
  ...
  INV upsert-orphan-guard: UpsertObservations silently DROPS rows whose series_id absent (AnyAsync gate) ¬error.
  INV TechnicalIndicator=scaffold: routable via admin/series-id but CollectionWorker has no branch → logs "Unknown series type" each cycle, ¬collected.
  INV migrations=raw-SQL: hand-written `migrations/*.sql` ¬EF (predates ATLAS ef_core convention); no ModelSnapshot.

PATHS (distinct code — do not conflate):
  collect [BackgroundService · CollectionWorker, 4h tick]
    do: holiday-skip(CME via CalendarService) → quota-check → scheduler picks ≤4 due (Priority↑→LastCollectedAt↑) → fetch → upsert → UpdateLastCollected(only if count>0). 500ms inter-series.
    does NOT: ¬register-SecMaster ¬emit-on-collect(stream polls independently) ¬retry-cycle(MaxRetries unused).
    on-miss: count==0 → ¬advance LastCollectedAt (re-picked next cycle); quota-exhausted → skip whole cycle; holiday → skip.
  SubscribeToEvents / GetEventsSince [gRPC :5001 · ThresholdEngine]
    ...
    on-miss: GetHealth=STUB(Healthy=true,TotalEvents=0,LatestEventTime=now) ¬real counts.
  ...
  search [HTTP :8080 /api/search · SecMaster gateway]
    do: local-DB title/symbol LIKE → {SeriesId,Title,Interval}. ¬upstream ¬quota.

PROCESSING MODEL:
  rate: token-bucket(5 cap, +1/min, queue 10) × hard daily-cap (DailyLimit=25/UTC-day). quota-exhausted → fetch returns []/null (¬throw). API "Note"/"Error Message" in body → []  (counts as used request).
  schedule: due = LastCollectedAt==null ∨ elapsed>interval-floor(daily 20h · weekly 6d · monthly 25d · quarterly 80d · annual 350d).

DISTINCTIONS:
  scalar-obs(Value, observations tbl) ≠ OHLCV(ohlcv tbl) — type-routed; stream+`/latest` branch on Type.
  series catalog (local AV) ≠ SecMaster instrument (registration is f-a-f advisory, ¬authoritative source).
...
CROSS-SERVICE: SecMaster ← register(gRPC f-a-f, OPTIONAL, on-add-only). ThresholdEngine → SubscribeToEvents(gRPC). CalendarService → CME holidays. OUT: AlphaVantage HTTP, OTEL :4317.
...
GOTCHAS:
  ...
  ✗ add-TechnicalIndicator-series (scaffold; "Unknown" log spam, ¬collected)
