...
- **Cleanup.** Teardown on `EXIT`, plus a reaper on every start that removes `atlas-*` state whose
  worktree is gone — SIGKILL cannot be trapped, so the trap alone would leak. A teardown that fails is
  retried, then reported as a `WARNING` on stderr; it never changes the run's exit status, because the
  tests and the marker are finished before it runs (nerdctl 1.7.7's `network rm` loses a race with any
  concurrent `run --rm` — the measurement is in `devcontainer-owner.sh` at `devcontainer_disown`).
