...
| `test_image_manifest.py` | `deployment/images.yml` through `deployment/scripts/image-lib.py check` -- AC3 of `docs/proposals/local-registry-image-promotion.md` | needs **pytest** and **pyyaml** (venv). `test_tracked_manifest_is_honest` is the live gate: every COPY/ADD source covered by a declared input, no input wider than what the build copies, and the row set equal to what `deploy.yml` builds from a repo Containerfile AND compose runs. The rest are known-bad controls over a copy of the REAL rows with ONE poisoned, so each finding must name its image among the clean ones. The build-image.sh controls live in `deployment/tests/promote/run.sh`. |
