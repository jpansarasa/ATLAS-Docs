...
### Operator re-resolution (writes production rows ONLY with `--live`)

| Script | Purpose |
|---|---|
| `reresolve-by-provenance.sh` | Drives `POST /admin/review/reresolve-by-instrument` (AGENT_README.md D-33), single-flight, 25 rows per request, appending every response to `RUN_DIR/responses.jsonl`; `--help` carries the eight-step run order. `--record-t-clean` TAKES T_clean after the SecMaster deploy: PR #1054's `SecMaster/scripts/junk-name-audit.sh --expect-bar 105 --allow CT="Cotton No. 2 Futures" --allow GC` must read rc 0, or rc 1 with GC's description_present as the only finding, and a stale-embedding SELECT (embedded_text not starting with the current name, or not newer than the row) must read 0 on two polls; otherwise nothing is recorded. Every mode that is part of the run (`--record-t-clean`, the dry run, `--live`) refuses a running sentinel-collector whose `/proc/1/environ` lacks `ReExtract__Enabled=false`. Every run's PREFLIGHT (rc 2) also refuses `--ids` outside the provenance SELECT, an `--attached-before` that is not the recorded T_clean, and stale embeddings; `--live` also refuses A2b - A2 > 0 (escaped rows: stop and report), no complete dry run for this T_clean and id set started from the beginning, any live-population row missing from that dry run's results, a CONTROL GATE below 70% (the DB/EVR/NMR rows whose subject is the old name #1053 recorded, by query) and any dry-run row landing on a suspect instrument (A10 before writes). STOPS (rc 1) on any non-2xx, any degraded row (naming the tier), any unavailable row, a move in `secmaster_rag_degraded_total` or (live) `sentinel_reextract_rows_processed_total`, and at `--max-requests`, printing the resume command. Its summary is NOT the verdict: `--after-checks` runs A2, A2b and A10 (rc 0 iff all 0). `--selftest` drives every rule with known-bad input AND runs the real preflight call site as a child on injected state (rc 2 unless each refuses by name and each clean case passes). |
