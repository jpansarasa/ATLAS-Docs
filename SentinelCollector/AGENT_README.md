...
DECISIONS:
  ...
  D-5 ingress-non-entity-classes: INTENT companion to D-1 (supersedes none) — extend the SAME ingress cleanup to the non-entity surface classes that flooded the paid Gemini resolver AFTER D-1 shipped (govt agency/regional-Fed, journalist byline, academic degree, garbled ticker-fragment); gating only the resolver destination (#823) is the recurring disease — the junk re-appears at the next consumer, so reject at the SOURCE and every consumer (FRED, Gemini, storage, matrix) is covered once / PRECOND reject only UNAMBIGUOUS non-instruments: curated federal-agency/stats-bureau set + trailing-"Fed" regional rule; political parties (trailing-"Party" only — "Party City" leads); govt institutions (standalone keyword Institute/Ministry/Directorate/Parliament/Politburo anywhere, corp-suffix-exempt); byline = embedded newline + all-caps dateline; degree/honorific token set with ticker-colliding tokens (BA=Boeing, MS=Morgan Stanley, MA=Mastercard, JD=JD.com, MD, BS) DELIBERATELY excluded; garbled = >=3 tokens containing a digit-bearing dotted uppercase ticker code. Bare/full public-official PERSON names (Carney/Warsh/Chuck Schumer) NOT rejected — no precise structural discriminator, a blocklist would nuke person-named issuers (Morgan Stanley, Charles Schwab); ambiguous names Keep -> SecMaster resolve + resolver daily-cap stay defense-in-depth / GUARD CandidateSurfaceFilter.Classify @ src/Services/CandidateSurfaceFilter.cs:266 / TEST EntityResolutionPrepassFilterTests.ingress_rejects_agency_byline_credential_and_garbled_but_keeps_real_entity
