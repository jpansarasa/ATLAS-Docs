...
DATA MODEL + INVARIANTS (schema/config does NOT enforce):
  ...
  INV route-table-divergence: appsettings.json (SHIPPED) = critical->[ntfy,email,autofix], warning->[ntfy,autofix], info->[ntfy]. C# `RoutingOptions` class default = critical->[ntfy,email], warning->[ntfy], info->[ntfy] (NO autofix). class default applies ONLY if `Routing` section absent entirely. never read class to know prod routing.
