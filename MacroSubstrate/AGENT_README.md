...
PURPOSE: shared EF write+read LIBRARY for `atlas_data.macro_observations` (canonical macro-obs store) + one-shot Migrator. Not a running service: no listener, no ports, no Program.cs main, does not own the projector.

DATA MODEL + INVARIANTS (schema does NOT enforce):
  D4 value-XOR: exactly-one(ValueNumeric XOR ValueQualitative); Trust non-null IFF qualitative. EnsureValid() client-side + DB CHECK (`ck_macro_obs_value_xor`, `ck_macro_obs_trust_only_qualitative`).
  INV idem-heal: write = INSERT...ON CONFLICT(source_collector,source_id,observation_time) DO **UPDATE** (heal-on-rewrite, last-write-wins), not DO-NOTHING. Top-level README says "DO NOTHING" — STALE; code does guarded DO UPDATE.
  INV unchanged-skip: DO UPDATE guarded by IS-DISTINCT-FROM on value-bearing cols -> byte-identical re-collect writes NOTHING; `WriteAsync->false` = unchanged-dup not error. id + idem-key cols never overwritten; ingestion_time refreshed on heal.
  ...
  INV soft-FK: SignalIdentityId (kebab text <=64) + AtlasSectorCode -> cross-DB `atlas_secmaster` -> PG does not enforce. owner-rec said UUID; actual col = text.
  INV SourceCollector = text not enum (new collector needs no migration).
  INV versioned-read⊥SecMaster-conn: SecMaster conn OPTIONAL at registration; absent -> write paths fine, as-of read paths throw `MappingVersionLookupUnavailableException`.
  INV write⊥version-lookup: IMappingVersionLookup nullable-injected; only as-of READ consults it.
...
PATHS (distinct code — do not conflate):
  WriteAsync / WriteBatchAsync [in-proc DI · collectors fred/ofr/sentinel via IMacroObservationWriter]
    do: EnsureValid -> parameterized upsert (heal-on-rewrite); batch = per-row single writes (one conflict does not poison siblings); return inserted/healed count.
  ...
  QueryAsync / GetObservationsAtDateAsync [in-proc DI · read consumers — ThresholdEngine ObservationCellProjector, Reports.Substrate]
    ...
    on-miss: as-of pre-history (no rollup covers date) -> EMPTY (contract, INFO not WARN); as-of AND no version-lookup -> MappingVersionLookupUnavailableException; null-label rows EXCLUDED from as-of slice.
  ...
  GetByIdempotencyKeyAsync [IMacroObservationRepository · point-lookup · tests + downstream confirmation]
    ...
    does NOT: paginate; filter; version.
  ...
  MigrateAsync [one-shot console `migrate-macro-substrate` · no listener]
    ...
    does NOT: run long; serve traffic. on-miss: missing conn-str | migration err -> exit 1 (loud-fail, full exception).
