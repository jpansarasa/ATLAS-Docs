...
A fixture service with a complete, read-first architecture card.

## ARCHITECTURE — GoodCardFixture   [read-first]
...
RESOLUTION MODEL ("fuzzy proposes, authoritative confirms"): local(exact→fuzzy→vector)→external PROPOSES → CONFIRM → persist OR review-queue. Empty = "exhausted every lookup", NOT "absent from table".
DISTINCTIONS:
...
SEE: README §Reference · EntityResolutionService.cs · ResolutionService.cs.
