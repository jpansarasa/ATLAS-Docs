...
precondition dies (gemini-resolver drain #823–825 — call-on-miss kept, rare∧hard∧
...
## ETHOS
judgment_gate ¬ mechanical_lint            # Approach C (lint+block) REJECTED — file:line stales in ordinary refactors
findings ¬ blocks                          # advisory; REVIEW_FIX_LOOP aggregates + dispatches fixes
D_entries = the contract                   # card DECISIONS block is what the diff is judged against
atomic_set change-all-or-none              # D-entry + INTENT comment + guard + guard-test move together
conflict → STOP ¬ route-around ¬ obey-stale
...
2. LOAD: for each touched service, read `<Service>/AGENT_README.md` DECISIONS block.
   `DECISIONS: none` ∨ no card → only CHECK_2 applies to that service.
...
## CHECKS
CHECK_1 guard_touched_without_supersession [critical]:
...
CHECK_2 new_exception_path_without_D_entry [critical if scarce-resource $/GPU/quota; important otherwise]:
...
CHECK_3 guard_test_missing_or_tautological [important]:
...
CHECK_4 intent_comment_orphaned [suggestion]:
...
Examples:
  ...
  past budget cap → refusal, ¬silent-pass
...
important:  CHECK_2 (non-scarce exception path) | CHECK_3 (missing ∨ tautological guard test)
...
## ANTI [HARD_STOP]
✗ block_mechanically # findings feed REVIEW_FIX_LOOP judgment; lint+block rejected (spec §OUT_OF_SCOPE)
✗ route_around_conflict | obey_stale_entry # STOP + report; a human/supervisor arbitrates
✗ accept_paraphrased_D_entries # judge against the card's VERBATIM entries, not the brief's summary
✗ demand_D_entries_for_ordinary_mechanism # scope discipline; DECISIONS: none is legal
✗ green_guard_test_after_guard_delete # the RED-on-removal property IS the test's reason to exist
✗ skip_because_card_missing # no card → CHECK_2 still applies; flag missing card as suggestion
...
¬declare_done UNTIL ALL:
  ...
  4. any CHECK_1 conflict surfaced as STOP verdict, ¬silently downgraded
